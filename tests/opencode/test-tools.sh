...
if echo "$output" | grep -qi "devpowers:brainstorming\|devpowers:using-devpowers\|Available skills"; then
    echo "  [PASS] find_skills tool discovered devpowers skills"
else
...
# Test 3: Test use_skill with devpowers: prefix
echo ""
echo "Test 3: Testing use_skill with devpowers: prefix..."
echo "  Running opencode with devpowers:brainstorming skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load devpowers:brainstorming and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "brainstorming\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded devpowers:brainstorming skill"
else
    echo "  [FAIL] use_skill did not load devpowers:brainstorming correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
