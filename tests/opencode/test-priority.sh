...
# Verifies that skills are resolved with correct priority: project > personal > devpowers
...
# 1. Create in devpowers location (lowest priority)
mkdir -p "$HOME/.config/opencode/devpowers/skills/priority-test"
cat > "$HOME/.config/opencode/devpowers/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Devpowers version of priority test skill
---
# Priority Test Skill (Devpowers Version)

This is the DEVPOWERS version of the priority test skill.

PRIORITY_MARKER_DEVPOWERS_VERSION
EOF
...
if [ -f "$HOME/.config/opencode/devpowers/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Devpowers version exists"
else
    echo "  [FAIL] Devpowers version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides devpowers
echo ""
echo "Test 2: Testing personal > devpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides devpowers)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_DEVPOWERS_VERSION"; then
    echo "  [FAIL] Devpowers version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|devpowers" | head -10
fi

# Test 3: Test that project overrides both personal and devpowers
echo ""
echo "Test 3: Testing project > personal > devpowers priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Devpowers version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit devpowers: prefix bypasses priority
echo ""
echo "Test 4: Testing devpowers: prefix forces devpowers version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load devpowers:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_DEVPOWERS_VERSION"; then
    echo "  [PASS] devpowers: prefix correctly forces devpowers version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] devpowers: prefix did not force devpowers version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
