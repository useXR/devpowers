...
# Create skills in personal and devpowers locations for testing
mkdir -p "$TEST_HOME/personal-skills/shared-skill"
mkdir -p "$TEST_HOME/devpowers-skills/shared-skill"
mkdir -p "$TEST_HOME/devpowers-skills/unique-skill"
...
cat > "$TEST_HOME/devpowers-skills/shared-skill/SKILL.md" <<'EOF'
---
name: shared-skill
description: Devpowers version
---
# Devpowers Shared
EOF

cat > "$TEST_HOME/devpowers-skills/unique-skill/SKILL.md" <<'EOF'
---
name: unique-skill
description: Only in devpowers
---
# Unique
EOF
...
const devpowersDir = '$TEST_HOME/devpowers-skills';
const personalDir = '$TEST_HOME/personal-skills';

// Test 1: Shared skill should resolve to personal
const shared = resolveSkillPath('shared-skill', devpowersDir, personalDir);
console.log('SHARED:', JSON.stringify(shared));

// Test 2: devpowers: prefix should force devpowers
const forced = resolveSkillPath('devpowers:shared-skill', devpowersDir, personalDir);
console.log('FORCED:', JSON.stringify(forced));

// Test 3: Unique skill should resolve to devpowers
const unique = resolveSkillPath('unique-skill', devpowersDir, personalDir);
console.log('UNIQUE:', JSON.stringify(unique));

// Test 4: Non-existent skill
const notfound = resolveSkillPath('not-a-skill', devpowersDir, personalDir);
console.log('NOTFOUND:', JSON.stringify(notfound));
" 2>&1)

if echo "$result" | grep -q 'SHARED:.*"sourceType":"personal"'; then
    echo "  [PASS] Personal skills shadow devpowers skills"
else
...
if echo "$result" | grep -q 'FORCED:.*"sourceType":"devpowers"'; then
    echo "  [PASS] devpowers: prefix forces devpowers resolution"
else
    echo "  [FAIL] devpowers: prefix not working"
    exit 1
fi

if echo "$result" | grep -q 'UNIQUE:.*"sourceType":"devpowers"'; then
    echo "  [PASS] Unique devpowers skills are found"
else
    echo "  [FAIL] Unique devpowers skills not found"
    exit 1
fi
