# Devpowers Release Notes

## v4.0.3 (2025-12-26)

### Improvements

**Strengthened using-devpowers skill for explicit skill requests**
...
The underlying skills (`devpowers:brainstorming`, `devpowers:executing-plans`, `devpowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-devpowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-devpowers`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `devpowers:brainstorming`)
...
**Skill priority in using-devpowers**
...
- **Rationalizations table** in using-devpowers - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-devpowers is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > devpowers
...
- Optimized devpowers bootstrap to eliminate redundant skill execution. The `using-devpowers` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-devpowers` manually despite already having the content from session start.
...
- Added unified `devpowers-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `devpowers:skill-name` for devpowers skills, `skill-name` for personal
- Personal skills override devpowers skills when names match
...
- Updated terminology: "Devpowers skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/devpowers-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/devpowers-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core devpowers functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-devpowers skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-devpowers/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-devpowers skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-devpowers/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `devpowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `devpowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `devpowers:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `devpowers:code-reviewer`
...
- All internal skill references now use `devpowers:` namespace prefix
- Updated format: `devpowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/devpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Devpowers v2.0.0 Release Notes

## Overview

Devpowers v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository. This transforms devpowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository.

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/devpowers/skills/`
...
1. Your old `~/.config/devpowers/.git` will be backed up to `~/.config/devpowers/.git.bak`
2. Old skills will be backed up to `~/.config/devpowers/skills.bak`
3. Fresh clone of skills will be created at `~/.config/devpowers/skills/`

### Removed Features

- **Personal devpowers overlay system** - Replaced with git branch workflow
- **setup-personal-devpowers hook** - Replaced by initialize-skills.sh
...
- `DEVPOWERS_SKILLS_ROOT` set to `~/.config/devpowers/skills`
...
- Removed obsolete setup-personal-devpowers call from session-start
...
**Removed:**
- `skills/` directory (82 files) - Now in separate repo
- `scripts/` directory - Now in skills/using-skills/
- `hooks/setup-personal-devpowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/devpowers/skills
- `commands/brainstorm.md` - Updated paths to DEVPOWERS_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to DEVPOWERS_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to DEVPOWERS_SKILLS_ROOT
...
- PR #2: Personal devpowers overlay system (later replaced)
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/devpowers/skills ~/devpowers-skills-backup
   ```
...
- Skills repository is now at https://github.com/arobb/devpowers
...
**Full Changelog:** https://github.com/arobb/devpowers/compare/dd013f6...main
