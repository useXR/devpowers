...
**Goal:** Add full devpowers support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/devpowers-codex` (lines 40-74)
...
- Reference: `.codex/devpowers-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'devpowers' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/devpowers-codex` (lines 212-280)
 ...
 * (personal skills override devpowers skills).
 *
 * @param {string} skillName - Name like "devpowers:brainstorming" or "my-skill"
 * @param {string} devpowersDir - Path to devpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, devpowersDir, personalDir) {
    // Strip devpowers: prefix if present
    const forceDevpowers = skillName.startsWith('devpowers:');
    const actualSkillName = forceDevpowers ? skillName.replace(/^devpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly devpowers:)
    if (!forceDevpowers && personalDir) {
    ...
    // Try devpowers skills
    if (devpowersDir) {
        const devpowersPath = path.join(devpowersDir, actualSkillName);
        const devpowersSkillFile = path.join(devpowersPath, 'SKILL.md');
        if (fs.existsSync(devpowersSkillFile)) {
            return {
                skillFile: devpowersSkillFile,
                sourceType: 'devpowers',
                skillPath: actualSkillName
            };
        }
    }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/devpowers-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/devpowers-codex` (add import at top)
...
Run: `node -c .codex/devpowers-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/devpowers-codex
...
**Files:**
- Modify: `.codex/devpowers-codex` (lines 40-74)
...
Run: `.codex/devpowers-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/devpowers-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/devpowers-codex` (lines 16-38, approximately)
...
Run: `.codex/devpowers-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/devpowers.js`
...
/**
 * Devpowers plugin for OpenCode.ai
 *
...
const devpowersSkillsDir = path.join(homeDir, '.config/opencode/devpowers/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const DevpowersPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/devpowers.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/devpowers.js
...
**Files:**
- Modify: `.opencode/plugin/devpowers.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "devpowers:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > devpowers)
...
Run: `node -c .opencode/plugin/devpowers.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the devpowers and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const devpowersSkills = skillsCore.findSkillsInDir(
            devpowersSkillsDir,
            'devpowers',
            3
          );
          ...
          const allSkills = [...personalSkills, ...devpowersSkills];

          if (allSkills.length === 0) {
            return 'No skills found. Install devpowers skills to ~/.config/opencode/devpowers/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'devpowers:';
...
```javascript
    'session.started': async () => {
      // Read using-devpowers skill content
      const usingDevpowersPath = skillsCore.resolveSkillPath(
        'using-devpowers',
        devpowersSkillsDir,
        personalSkillsDir
      );

      let usingDevpowersContent = '';
      if (usingDevpowersPath) {
        const fullContent = fs.readFileSync(usingDevpowersPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingDevpowersContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Devpowers skills: \`devpowers:skill-name\` (from ~/.config/opencode/devpowers/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/devpowers')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/devpowers && git pull` to update devpowers.' :
        '';
...
**Below is the full content of your 'devpowers:using-devpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**

${usingDevpowersContent}
...
```markdown
# Installing Devpowers for OpenCode
...
### 1. Install Devpowers Skills

```bash
# Clone devpowers skills to OpenCode config directory
mkdir -p ~/.config/opencode/devpowers
git clone https://github.com/obra/devpowers.git ~/.config/opencode/devpowers
```

### 2. Install the Plugin

The plugin is included in the devpowers repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/devpowers/.opencode/plugin/devpowers.js`
...
ln -s ~/.config/opencode/devpowers/.opencode/plugin/devpowers.js .opencode/plugin/devpowers.js
```
...
1. Verify skills directory exists: `ls ~/.config/opencode/devpowers/skills`
...
- Report issues: https://github.com/obra/devpowers/issues
- Documentation: https://github.com/obra/devpowers
```
...
Devpowers works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/devpowers-codex use-skill devpowers:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/devpowers-codex bootstrap | head -30`
...
ls -l .opencode/plugin/devpowers.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/devpowers.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/devpowers-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/devpowers-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/devpowers.js` created with tools and hooks
