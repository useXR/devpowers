...
Add full devpowers support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port devpowers to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/devpowers-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/devpowers.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/devpowers/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/devpowers-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "devpowers:brainstorming")')
  }),
...
1. **Inject using-devpowers content**
   - Full content of the using-devpowers skill
...
export const DevpowersPlugin = async ({ client, directory, $ }) => {
  const devpowersDir = path.join(process.env.HOME, '.config/opencode/devpowers');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingDevpowers = await readSkill('using-devpowers');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingDevpowers}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── devpowers-codex        # UPDATED: Use skills-core
│   ├── devpowers-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── devpowers.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/devpowers-codex`
...
2. Update `.codex/devpowers-codex` to use shared core
...
1. Create `.opencode/plugin/devpowers.js`
